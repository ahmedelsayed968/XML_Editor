    ...
    def __printJSON(self, root: Node, level=0, temp: list = [], temp1: list = []):
        ...
        if (length == 0): 
            ...
            if (level == 0):
                ...
                self.outputstring+=sep + "\"" + root.tag_name + "\":{\n"
                if root.has_attribute:
                    self.outputstring+=sep+"\t"+root.attribute_name + "\":" + root.attribute_value + "\n"
                else:
                    self.outputstring+=root.children[0].value
                        # self.outputstring+="{\n"
                ...
                if root.is_open_tag:
                    self.outputstring+=sep + "\"" + root.tag_name + "\":"
                    if root.has_attribute:
                         self.outputstring+="\n" + sep+"\t"+root.attribute_name + "\":" + root.attribute_value + "\n"
                    else:
                        self.outputstring+=root.children[0].value
                        self.outputstring+="[\n"
                        temp.append(root.tag_name)
...
<catalog>
   ...
   <book id="bk112">
      ...
      environment.</description>
            ...
            """
        xmlTree = Tree()
        xmlTree.parser(input_str)
        xmlTree.correter_XML()
        #print(xmlTree.visualizeXML())
