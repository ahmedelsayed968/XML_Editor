...
class HuffmanCode:
    def __init__(self, data: str):
        ...
        self.heap = []
        self.codes = Dictionary()
        self.reversecode = Dictionary()
    ...
    def __freq_dict_builder(self,xml_text):
        ...
        for character in xml_text:
            if not character in frequency.keys:
                frequency.add(character, 0)
            frequency.add(character, frequency.get(character) + 1)
        return frequency
    
    # build heap , push minimum nodes into heap
    def __heap_builder(self, frequency):
        for key in frequency.keys:
            node = self.BinaryTreeNode(key, frequency.get(key))
    ...
    def __codes_generator_helper(self, root, current_code):
        if root == None:
            return
        if root.char != None:
            self.codes.add(root.char, current_code)
            self.reversecode.add(current_code, root.char)
            return
    ...
    def __encoded_text_builder(self, text):
        ...
        for character in text:
            encoded_text += self.codes.get(character)
        return encoded_text
    ...
    def __xml_text_decoder(self, encoded_text):
        ...
        for bit in encoded_text:
            current_code += bit
            if self.reversecode.contains(current_code):
                character = self.reversecode.get(current_code)
